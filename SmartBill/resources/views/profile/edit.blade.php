    ...
    <div x-data="{ activeTab: 'account' }" class="flex flex-col md:flex-row min-h-screen md:min-h-[calc(100vh-64px)] bg-[#fafafa] dark:bg-[#1e1f22] animate-in fade-in duration-700 pb-20 md:pb-0 font-sans tracking-tight relative">
        ...
        <div class="w-full md:w-[280px] lg:w-[320px] p-6 md:p-10 shrink-0 flex flex-col gap-2 md:h-[calc(100vh-64px)] md:sticky md:top-0 border-r border-[#e3e5e8]/50 dark:border-[#313338]/50 overflow-y-auto">
            ...
            <button @click="activeTab = 'billing'" 
                ...
                <i class="bi bi-credit-card-fill w-4 h-4 transition-transform group-hover:scale-110"></i> Billing & Tokens
            </button>
        ...
        <div class="flex-1 p-6 md:p-12 lg:p-16 w-full">
            ...
            <h2 class="hidden md:block text-2xl md:text-3xl font-black text-[#1e1f22] dark:text-white tracking-tight mb-10 transition-all duration-300" x-text="activeTab === 'account' ? 'Profile Details' : (activeTab === 'appearance' ? 'Interface Settings' : (activeTab === 'security' ? 'Security & Access' : 'Billing & Tokens'))"></h2>
            ...
            <div x-show="activeTab === 'billing'" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-cloak class="space-y-8" style="display: none;">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="p-6 rounded-xl bg-white dark:bg-[#2b2d31] border border-black/[0.05] dark:border-white/5 shadow-sm">
                        <div class="text-[9px] font-black uppercase tracking-[0.2em] text-[#80848e] mb-3">Current Balance</div>
                        <div class="flex items-center gap-3">
                            <i class="bi bi-coin text-discord-green text-2xl"></i>
                            <div class="text-2xl font-black text-[#1e1f22] dark:text-white uppercase">{{ number_format($user->tokens) }} <span class="text-[10px] text-[#80848e]">Tokens</span></div>
                        </div>
                    </div>
                    <div class="p-6 rounded-xl bg-white dark:bg-[#2b2d31] border border-black/[0.05] dark:border-white/5 shadow-sm">
                        <div class="text-[9px] font-black uppercase tracking-[0.2em] text-[#80848e] mb-3">Usage This Month</div>
                        <div class="flex items-center gap-3">
                            <i class="bi bi-graph-up-arrow text-indigo-500 text-2xl"></i>
                            <div class="text-2xl font-black text-[#1e1f22] dark:text-white uppercase">{{ number_format($usageThisMonth) }} <span class="text-[10px] text-[#80848e]">Slips</span></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-[#06C755]/5 to-transparent p-8 rounded-xl border border-[#06C755]/20 shadow-sm flex flex-col md:flex-row items-center gap-8">
                    ...
                    <div class="flex-1 text-center md:text-left">
                        <h4 class="text-lg font-black text-[#1e1f22] dark:text-white mb-1">เติมโทเค็นผ่าน LINE Official</h4>
                        <p class="text-xs font-bold text-[#5c5e66] dark:text-[#b5bac1] leading-relaxed">ติดต่อเจ้าหน้าที่เพื่อดำเนินการเติมโทเค็นเข้าสู่ระบบได้ทันที</p>
                    </div>
                ...
                <div class="space-y-6 pb-10">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="text-xs font-black uppercase tracking-[0.2em] text-[#1e1f22] dark:text-white">Usage History</h3>
                        </div>
                        <span class="text-[9px] font-bold text-[#80848e] uppercase tracking-widest bg-black/[0.03] dark:bg-white/[0.03] px-2 py-1 rounded-lg">Grouped by hour</span>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody class="divide-y divide-black/5 dark:divide-white/5 text-[11px] font-bold">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-4 py-10 text-center text-[#80848e] italic uppercase tracking-widest text-[9px]">No transactions yet</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
