        ...
        <div class="premium-card p-6 md:p-8 border-l-4 border-l-rose-500">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div class="inline-flex items-center gap-3 px-5 py-3 rounded-[18px] bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.2em]">
                    <i data-lucide="coins" class="w-4 h-4"></i> {{ number_format($user->tokens) }} Tokens Ready
                </div>
            </div>
        </div>

        <div class="rounded-[2rem] border border-slate-200 bg-slate-50 px-5 py-4 text-sm font-bold text-slate-600 dark:border-white/10 dark:bg-white/[0.03] dark:text-slate-300">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-[1.2fr_0.8fr] gap-6">
            <div class="premium-card p-6 md:p-8">
                ...
                @if($tokenLogs->isEmpty())
                    <div class="rounded-[24px] border-2 border-dashed border-slate-200 dark:border-white/10 p-10 text-center text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400">
                        No token activity yet
                    </div>
                @else
                    <div class="space-y-3">
                        @foreach($tokenLogs as $log)
                            <div class="flex items-center justify-between gap-4 rounded-[20px] border border-slate-100 dark:border-white/5 bg-slate-50/70 dark:bg-white/[0.03] px-5 py-4">
                                <div>
            ...
            <div class="space-y-6">
                <div class="premium-card p-6 md:p-8">
                    ...
                    <form method="POST" action="{{ route('billing.topups.store') }}" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="requested_tokens" min="1" step="1" value="100" required class="w-full rounded-[16px] border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 dark:border-white/10 dark:bg-[#1e1f22] dark:text-white">
                        </div>
                        <div>
                            <label class="mb-2 block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Transferred Amount</label>
                            <input type="number" name="amount_paid" min="0" step="0.01" placeholder="Optional" class="w-full rounded-[16px] border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 dark:border-white/10 dark:bg-[#1e1f22] dark:text-white">
                        </div>
                        <div>
                            <label class="mb-2 block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Payment Slip</label>
                            <input type="file" name="payment_slip" accept="image/*" required class="w-full rounded-[16px] border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 file:mr-4 file:rounded-full file:border-0 file:bg-discord-green file:px-4 file:py-2 file:text-xs file:font-black file:uppercase file:tracking-[0.2em] file:text-white dark:border-white/10 dark:bg-[#1e1f22] dark:text-white">
                        </div>
                        <div>
                            <label class="mb-2 block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Note</label>
                            <textarea name="note" rows="3" class="w-full rounded-[16px] border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-800 dark:border-white/10 dark:bg-[#1e1f22] dark:text-white" placeholder="Optional note for admin"></textarea>
                        </div>
                ...
                <div class="premium-card p-6 md:p-8">
                    ...
                    <div class="space-y-3">
                        <div class="rounded-[20px] border border-slate-100 bg-slate-50/70 px-5 py-4 text-sm font-bold text-slate-600 dark:border-white/5 dark:bg-white/[0.03] dark:text-slate-300">
                ...
                <div class="premium-card p-6 md:p-8">
                    ...
                    <div class="space-y-3">
                        ...
                        @empty
                            <div class="rounded-[20px] border border-dashed border-slate-200 px-5 py-6 text-center text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400 dark:border-white/10">
