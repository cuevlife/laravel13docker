...
<head>
    ...
    <title>Register - {{ config('app.name', 'SmartBill') }}</title>
...
<body class="bg-[#fafafa] dark:bg-[#1e1f22] font-sans tracking-tight text-slate-900 dark:text-slate-100">
    <div class="flex flex-col lg:flex-row min-h-screen sm:min-h-[600px] bg-white dark:bg-discord-main border-0 sm:border border-slate-100 dark:border-white/5 overflow-hidden transition-all duration-500">
        ...
        <!-- SIDE B: Register Form -->
        <div class="w-full lg:w-1/2 p-10 sm:p-16 flex flex-col justify-between bg-white dark:bg-transparent">
            
            <div class="flex-1 flex flex-col justify-center">
                <div class="w-full max-w-sm mx-auto">
                    <div class="mb-12 text-center lg:text-left">
                        <h2 class="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tighter leading-none">{{ __('Register') }}</h2>
                        <p class="mt-3 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.4em] opacity-60">Create New Pipeline</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('register') }}" class="space-y-8">
                        ...
                        <div class="relative group">
                            <label class="absolute -top-6 left-0 text-[10px] font-black text-slate-400 uppercase tracking-widest group-focus-within:text-discord-green transition-colors">{{ __('Name') }}</label>
                            <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus 
                                   class="block w-full px-0 py-3 bg-transparent border-0 border-b-2 border-slate-100 dark:border-white/10 text-lg font-black text-slate-900 dark:text-white focus:ring-0 focus:border-discord-green transition-all outline-none placeholder-slate-200 dark:placeholder-slate-800">
                            @if($errors->has('name'))
                                <p class="mt-2 text-[10px] font-bold text-discord-red uppercase">{{ $errors->first('name') }}</p>
                            @endif
                        </div>

                        <div class="relative group">
                            <label class="absolute -top-6 left-0 text-[10px] font-black text-slate-400 uppercase tracking-widest group-focus-within:text-discord-green transition-colors">{{ __('Email') }}</label>
                            <input id="email" type="email" name="email" value="{{ old('email') }}" required 
                                   class="block w-full px-0 py-3 bg-transparent border-0 border-b-2 border-slate-100 dark:border-white/10 text-lg font-black text-slate-900 dark:text-white focus:ring-0 focus:border-discord-green transition-all outline-none placeholder-slate-200 dark:placeholder-slate-800">
                            @if($errors->has('email'))
                                <p class="mt-2 text-[10px] font-bold text-discord-red uppercase">{{ $errors->first('email') }}</p>
                            @endif
                        </div>
                        ...
                        <div class="relative group">
                            <label class="absolute -top-6 left-0 text-[10px] font-black text-slate-400 uppercase tracking-widest group-focus-within:text-discord-green transition-colors">{{ __('Confirm Password') }}</label>
                            <input id="password_confirmation" type="password" name="password_confirmation" required 
                                   class="block w-full px-0 py-3 bg-transparent border-0 border-b-2 border-slate-100 dark:border-white/10 text-lg font-black text-slate-900 dark:text-white focus:ring-0 focus:border-discord-green transition-all outline-none placeholder-slate-200 dark:placeholder-slate-800">
                            @if($errors->has('password_confirmation'))
                                <p class="mt-2 text-[10px] font-bold text-discord-red uppercase">{{ $errors->first('password_confirmation') }}</p>
                            @endif
                        </div>

                        <div class="pt-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                            <a class="text-[10px] font-black text-slate-400 dark:text-slate-500 hover:text-discord-green dark:hover:text-discord-green uppercase tracking-widest transition-colors" href="{{ route('login') }}">
                                {{ __('Already registered?') }}
                            </a>

                            <button type="submit" class="w-full sm:w-auto px-8 py-4 bg-discord-green hover:bg-[#1a8348] text-white font-black rounded-2xl shadow-xl shadow-emerald-900/30 transition transform active:scale-[0.96] text-xs uppercase tracking-[0.2em]">
                                {{ __('Register') }}
                            </button>
                        </div>
                    </form>
